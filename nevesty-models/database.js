...
async function initDatabase() {
  ...
  await run(`ALTER TABLE models ADD COLUMN telegram_chat_id TEXT DEFAULT NULL`).catch(() => {});
  ...
  // Schema v41 — FAQ enhancements: keywords + use_count (БЛОК 34)
  const v41 = await get(`SELECT version FROM schema_versions WHERE version=41`).catch(() => null);
  if (!v41) {
    ...
    await run(`CREATE INDEX IF NOT EXISTS idx_faq_category ON faq(category, active)`).catch(() => {});
    await run(`CREATE INDEX IF NOT EXISTS idx_faq_use_count ON faq(use_count DESC)`).catch(() => {});
    await run(
      `INSERT OR IGNORE INTO schema_versions(version, description) VALUES(41, 'faq: keywords JSON array + use_count for auto-answer matching (БЛОК 34)')`
    ).catch(() => {});
  }
  // Idempotent — ensure columns exist even if migration was skipped
  await run(`ALTER TABLE faq ADD COLUMN keywords TEXT DEFAULT '[]'`).catch(() => {});
  await run(`ALTER TABLE faq ADD COLUMN use_count INTEGER DEFAULT 0`).catch(() => {});
  ...
  if (!faqCount.n) {
    const FAQ_SEED = [
      {
        question: 'Сколько стоит аренда модели?',
        answer:
          'Стоимость зависит от типа съёмки и длительности. Минимальный бюджет — от 8 000 ₽/час. Для точного расчёта заполните форму заявки — менеджер свяжется в течение часа.',
        keywords: '["цена","стоимость","сколько","тариф","прайс","расценки","стоит","дорого","дешево","бюджет"]',
        category: 'pricing',
      },
      {
        question: 'Как оформить заявку?',
        answer:
          'Нажмите кнопку «📋 Подать заявку» в главном меню, выберите тип события и заполните форму. Менеджер подтвердит заявку в течение 1–2 часов.',
        keywords: '["заявка","бронь","забронировать","оформить","записаться","как заказать","заказать"]',
        category: 'process',
      },
      {
        question: 'Как долго обрабатывается заявка?',
        answer:
          'Обычно в течение 2 часов в рабочее время (9:00–22:00). Срочные запросы рассматриваются в течение 30 минут.',
        keywords: '["долго","ждать","срок","когда","ответ","время","быстро","срочно","ожидание"]',
        category: 'process',
      },
      {
        question: 'Можно ли выбрать конкретную модель?',
        answer:
          'Да! В каталоге выберите понравившуюся модель и нажмите «📋 Забронировать» прямо на её карточке. Мы постараемся выполнить ваш выбор.',
        keywords: '["выбрать","конкретная","определённая","та же","нравится","каталог","портфолио"]',
        category: 'process',
      },
      {
        question: 'Какие форматы съёмок доступны?',
        answer:
          'Мы работаем с фотосъёмками, видеосъёмками, промо-акциями, выставками, показами и корпоративными мероприятиями. Уточните формат при подаче заявки.',
        keywords: '["съёмка","фото","видео","формат","виды","мероприятие","акция","выставка","показ","корпоратив"]',
        category: 'shooting',
      },
      {
        question: 'Работаете ли вы в выходные и праздники?',
        answer: 'Да, мы принимаем заявки 7 дней в неделю. Менеджеры онлайн с 9:00 до 22:00, включая праздники.',
        keywords: '["выходные","праздники","суббота","воскресенье","нерабочий","выходной"]',
        category: 'process',
      },
      {
        question: 'Как оплатить услуги?',
        answer:
          'Принимаем оплату на карту, переводом, наличными и через Telegram Stars. Предоплата — 50% при подтверждении заявки, остаток — по факту.',
        keywords: '["оплата","платить","оплатить","карта","перевод","наличные","предоплата","счёт","реквизиты"]',
        category: 'pricing',
      },
      {
        question: 'Какие гарантии качества?',
        answer:
          'Все модели прошли профессиональный отбор. Средний рейтинг по отзывам клиентов — 4.8/5. При несоответствии ожиданиям — возврат средств в течение 24 часов.',
        keywords: '["гарантии","качество","надёжность","отзывы","возврат","ответственность","проверены"]',
        category: 'process',
      },
      {
        question: 'Есть ли доставка модели?',
        answer:
          'Модели работают в вашем городе или приезжают к месту съёмки. Стоимость проезда за пределы города оговаривается отдельно.',
        keywords: '["доставка","привезти","приехать","приедет","транспорт","дорога","локация","адрес"]',
        category: 'delivery',
      },
      {
        question: 'Можно ли получить промокод или скидку?',
        answer:
          'Да! Постоянным клиентам и по реферальной программе предоставляются скидки. Введите промокод при оформлении заявки или спросите менеджера.',
        keywords: '["промокод","скидка","акция","дисконт","бонус","скидки","реферал","ещё дешевле"]',
        category: 'pricing',
      },
    ];
    for (const item of FAQ_SEED) {
      await run(
        ...
        [item.question, item.answer, item.keywords, item.category]
      ).catch(() => {});
    }
  }
