...
if (!TOKEN || TOKEN === 'your_bot_token_here') {
  console.error('❌ TELEGRAM_BOT_TOKEN не задан в .env'); process.exit(1);
}
if (!ADMIN_ID) {
  console.error('❌ ADMIN_TELEGRAM_IDS не задан в .env'); process.exit(1);
}
...
async function main() {
  ...
  for (const m of models) {
    let urls = [];
    try { urls = JSON.parse(m.photos || '[]'); } catch {}
