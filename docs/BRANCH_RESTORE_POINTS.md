...
Сейчас `main` — снимок от 21 июля: 797 файлов против 1246 в стволе, без всей
работы за август-сентябрь. Любой, кто открывает репозиторий по умолчанию, видит
устаревший проект. Привести в порядок можно двумя способами, и оба требуют
решения владельца, потому что затрагивают деплой:

1. перевести деплой Railway на `main` и сделать её рабочей веткой;
2. либо оставить деплой как есть, а `main` привести к стволу, чтобы она перестала
   вводить в заблуждение.

Истории `main` и ствола **не имеют общего предка**, поэтому обычным merge это не
делается — нужен либо `--allow-unrelated-histories`, либо принудительное
обновление. Сам не трогал: это операция с веткой по умолчанию.
