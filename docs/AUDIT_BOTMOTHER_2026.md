> **ИСТОРИЧЕСКИЙ ДОКУМЕНТ.** Снимок состояния на 2026-06-20. Часть находок с
> тех пор закрыта (платёж за strike активирует доступ, идемпотентность шины,
> Ban Weather доведён до фазы 3), а разделы про `platform/` описывают дерево,
> удалённое из репозитория 2026-09-04 (см. `docs/adr/0001-one-kernel.md`).
> Не используйте как описание текущего состояния — только как запись о том,
> что и когда было найдено.
