...
- **Ретраи Telegram API** (`telegram_api.call`) — сетевые сбои повторяются с
  backoff, flood-control 429 ждёт `retry_after`.
- **Ошибки Claude API** (`_claude_turn`) — лимиты, 5xx, обрыв связи и неверный
  ключ превращаются в понятное сообщение в чат, а не в исключение.
- **Самовосстановление контекста** — 400 от API (повреждённая история, смена
  модели) → автоматическая очистка контекста и повтор запроса.
