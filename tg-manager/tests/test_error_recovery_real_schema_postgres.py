...
СНАЧАЛА ГЛАВНОЕ: модуль `services/error_recovery` НЕ ПОДКЛЮЧЁН (он в списке
известных сирот в tests/test_no_orphan_services.py — дублирует предохранитель
op_worker и повторы operation_bus). То есть эти запросы не падали в проде —
они не выполнялись вовсе. Чинить их всё равно нужно: неподключённый модуль
выглядит рабочим, и тот, кто решит его подключить, получит три запроса, которые
не могут выполниться никогда. Тест фиксирует поведение на случай подключения и
не даёт снова написать SQL по воображаемой схеме.
