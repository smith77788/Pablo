...
def test_web_role_starts_no_background_loops():
    ...
    # Выход ДО запуска сервиса, а не после. Раньше здесь стоял якорь
    # `while True` — сам цикл присмотра. Его вынесли в
    # services/service_supervisor.py (вложенной функцией он был непроверяем, и
    # там пряталось зависание процесса на сервисе, выключенном настройкой).
    # Якорем стал вызов присмотра: это и есть точка, после которой сервис
    # начинает работать.
    assert body.index('_ROLE == "web"') < body.index("supervise("), \
        "проверка роли стоит после запуска сервиса — он всё равно стартует"
