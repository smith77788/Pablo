...
KNOWN_ORPHANS = {
    ...
    "bot_mesh",            # ядро Bot Mesh построено раньше транспорта: приём/
                           # отправка bot-message требует opt-in в @BotFather на
                           # живых ботах. Гашение петель безопаснее иметь до
                           # транспорта. Подключить с хендлерами bot-message.
}
