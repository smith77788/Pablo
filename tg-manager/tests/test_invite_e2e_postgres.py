...
def test_closed_group_stops_the_fleet(stand):
    ...
    try:
        ...
        # Закрытая группа замечается после первого ПАРАЛЛЕЛЬНОГО чанка (до
        # _parallel аккаунтов бьют по ней одновременно), поэтому граница — один
        # круг флота (≤ аккаунтов×batch_size = 2×5), а не один батч. Главное —
        # флот НЕ идёт по второму кругу: без остановки было бы кратно больше.
        assert len(stand.sent) <= 10, "флот не должен разбиваться об закрытую группу кругами"
