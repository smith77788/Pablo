...
# acc#<id>, что у жалобы. Самый дорогой случай — подписчики: повторный
# joinChannel туда, где аккаунт уже состоит, Telegram считает в лимит вступлений
# и в давление, ведущее к PEER_FLOOD, то есть повтор после сетевого блипа сам по
# себе поднимал риск бана. У просмотров, историй, реакций и запусков бота
# результат от повтора не растёт (реакция задаётся списком, а не переключается),
# поэтому там повтор — чистый расход вызовов и лимита аккаунта.
GUARDED = {
