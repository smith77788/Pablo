...
не меняй. Твоя задача — подобрать КАК МОЖНО БОЛЬШЕ синонимов к словам и оборотам
этого текста, чтобы из него получился максимально рандомизированный шаблон
рассылки. Telegram банит за одинаковые сочетания слов, поэтому чем больше разных
синонимов и чем плотнее покрытие — тем лучше.

Верни СТРОГО JSON такого вида:
{{"variants": [ [ {{"orig": "<точная подстрока из текста>", "syn": ["синоним1", "синоним2", ...]}}, ... ], ... ]}}

ЦЕЛЬ №1 — ПЛОТНОЕ ПОКРЫТИЕ:
- Пройди по тексту слева направо и дай синонимы почти к КАЖДОМУ значимому слову
  и обороту: глаголам, существительным, прилагательным, наречиям, приветствиям,
  вводным словам, устойчивым выражениям.
- НЕ должно оставаться подряд БОЛЬШЕ 2 слов без своей группы синонимов. Если между
  твоими записями остаётся 3+ слова подряд без замен — добавь ещё записи для слов
  из этого куска (или возьми оборот целиком в "orig").
- Служебные слова (предлоги, союзы, частицы), имена собственные и цифры можно
  пропускать — но их не должно быть 3+ подряд без окружения из групп.

ЦЕЛЬ №2 — МНОГО СИНОНИМОВ:
- Давай 4-8 синонимов на каждую группу (минимум 3). Чем больше живых, разговорных
  вариантов — тем лучше. Пример: «Здравствуйте» → «Привет», «Приветствую»,
  «Добрый день», «День добрый», «Доброго времени суток».
- Бери и целые ОБОРОТЫ (2-4 слова) в "orig" и давай к ним синонимичные обороты:
  «ваш ответ» → «вашего ответа», «ваше сообщение», «вашего сообщения».

ЦЕЛЬ №3 — ГРАММАТИКА И СМЫСЛ (нарушение = брак, важнее богатства):
- Синоним строго в ТОЙ ЖЕ форме, что и orig, чтобы фраза не сломалась ни в одной
  комбинации: та же часть речи; тот же падеж, род, число; у глаголов — тот же ВИД
  (сов./несов.), ВРЕМЯ и НАКЛОНЕНИЕ (инфинитив→инфинитив «спросить»→«уточнить»,
  НЕ «спроси»; прош. сов.→прош. сов. «увидела»→«заметила», НЕ «замечала»).
- РЕГИСТР обращения: текст на «ты» — все синонимы на «ты» («позволь»→«разреши»);
  текст на «вы» — только на «вы». НИКОГДА не смешивай «ты» и «вы» в одном тексте.
- Смысл сохраняй: не заменяй на близкое-но-другое («увидела»→«восприняла» — брак).
- Управление (предлоги/падеж) синонима = как у orig. Если orig с большой буквы —
  синонимы тоже с большой.
- Только язык оригинала. Ни одного иностранного слова, ни одной чужой буквы.
- Лучше пропустить слово, чем дать синоним, который где-то ломает форму/смысл.
...
- В "variants" ровно {count} набора; наборы должны заметно отличаться подбором
  синонимов (по возможности не повторяй один и тот же синоним первым).
