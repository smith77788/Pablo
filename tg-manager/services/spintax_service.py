...
ЛИМИТ НА ГРУППУ: на каждое слово/оборот давай РОВНО 1 или 2 синонима — не больше.
В готовом шаблоне это даст группу {{orig|син1}} или {{orig|син1|син2}} (2-3
варианта). Больше 3 вариантов в группе — НЕЛЬЗЯ. Выбирай 1-2 САМЫХ точных.
...
- На группу — 1 или 2 синонима (максимум 3 варианта с оригиналом), не больше.
  Обороты (2-4 слова) тоже бери в "orig" целиком.
...
def _clean_synonyms(orig: str, syns: list, script: str, register: str | None = None) -> list[str]:
    ...
    for raw in syns:
        ...
        if orig_cap and s[:1].islower():
            ...
            s = s[:1].lower() + s[1:]
        out.append(s)
        if len(out) >= MAX_VARIANTS_PER_GROUP - 1:  # orig + до 2 синонимов = максимум 3 варианта
            break
    return out
