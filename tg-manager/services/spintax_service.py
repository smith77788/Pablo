...
- "syn" — 2-5 синонимов, которые можно подставить ВМЕСТО orig, и фраза останется
  грамотной, естественной и с тем же смыслом.
- 🔑 ГЛАВНОЕ ПРАВИЛО — синоним должен быть в ТОЧНО ТОЙ ЖЕ грамматической форме,
  что и orig, чтобы предложение не сломалось. Совпадать должны:
  • часть речи;
  • падеж, род, число;
  • у глаголов — ВИД (совершенный/несовершенный), ВРЕМЯ и НАКЛОНЕНИЕ:
    инфинитив меняй только на инфинитив («спросить»→«поинтересоваться», НЕ «спроси»);
    повелительное — только на повелительное того же лица/числа;
    прошедшее совершенное — только на прошедшее совершенное
    («увидела»→«заметила», НЕ «замечала»);
  • ⚠️ РЕГИСТР обращения: если текст на «ты» — все синонимы на «ты»
    («позволь»→«разреши», НЕ «позвольте/разрешите»); если на «вы» — только на «вы».
    НИКОГДА не смешивай «ты» и «вы».
- Смысл сохраняй буквально: не заменяй слово на близкое, но другое по значению
  («увидела»→«восприняла» — ЗАПРЕЩЕНО).
- Пиши синонимы ТОЛЬКО на языке текста. НИ ОДНОГО иностранного слова, ни одной
  буквы другого алфавита (никакой латиницы, никаких иероглифов вроде 背景).
- КАЧЕСТВО ВАЖНЕЕ ПОКРЫТИЯ: лучше оставить слово без синонимов, чем дать вариант,
  который меняет форму, регистр («ты/вы») или смысл. Кривой синоним недопустим.
- Стремись к хорошему покрытию — синонимы к глаголам, существительным,
  прилагательным, наречиям и устойчивым оборотам, — но только там, где есть
  действительно взаимозаменяемые в этой форме слова.
- Если orig с большой буквы — синонимы тоже с большой. Следи за управлением
  (предлогами): синоним должен требовать тот же предлог/падеж.
- Обороты (2-4 слова) тоже бери в "orig" целиком, если у них есть хороший синоним.
- НЕ перекрывай orig-подстроки между собой.
- Если к слову нет 2+ живых синонимов В ТОЙ ЖЕ ФОРМЕ — пропусти его.
