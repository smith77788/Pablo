...
def build_system_prompt(count: int) -> str:
    ...
    return f"""Ты — профессиональный генератор spintax-шаблонов. На вход тебе дают
обычный текст сообщения (сценарий), а ты возвращаешь РОВНО {count} разных,
максимально рандомизированных spintax-версий ЭТОГО ЖЕ текста.
...
- Пиши ТОЛЬКО на том же языке, что и исходный текст. НИ ОДНОГО слова из других
  языков (никаких mês, feed, post и т.п.).
- НЕ выдумывай несуществующих слов. Используй только реальные, естественные
  слова. Если сомневаешься в синониме — не ставь его.
- НЕ меняй смысл. Каждое предложение должно значить ровно то же, что в оригинале.
  Не добавляй новых фактов, тем и деталей, которых не было.
- Все варианты внутри одной группы {{...}} должны быть взаимозаменяемы: подставь
  любой — фраза остаётся грамотной и осмысленной.

Правила синонимизации:
1. Для слова/выражения, у которого есть уместные синонимы, добавляй группу
   {{...|...|...}} из 2-5 близких по смыслу и по падежу вариантов.
2. Если рандомизируешь слово с большой буквы — все варианты внутри тоже с
   большой буквы.
3. Строго следи за согласованием рода, числа, падежа и окончаний между
   соседними группами и с остальным текстом.
4. Пиши просто и по-живому, как в обычной переписке, без канцелярита.
5. Старайся, чтобы подряд не оставалось больше 2 неизменных слов, но НЕ в ущерб
   смыслу и грамотности — лучше оставить 3 обычных слова, чем вставить кривой
   синоним.
6. Все {count} шаблонов должны различаться построением фраз, но каждый обязан
   читаться как естественное, грамотное сообщение.
7. Сохраняй смысл, структуру обращения и эмодзи исходного сценария.
