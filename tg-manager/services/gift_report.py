...
class GiftTransferReportService:
    ...
    @staticmethod
    async def generate_report(pool, plan_id: int, operation_id: Optional[int] = None) -> int:
        ...
        for acc_id in account_ids:
            ...
            if acc:
                gifts_count = sum(1 for i in plan["items"] if i["account_id"] == acc_id and i["status"] == "transferred")
        ...
        total_cost = sum(
            i.get("stars_cost", 0) for i in plan["items"] if i["status"] == "transferred"
        )
        ...
        report = await pool.fetchrow("""
            ...
            RETURNING id
        """,
            plan_id, operation_id, plan["owner_id"],
            inventory_count, total_selected, transferred, failed, skipped, pending,
            total_cost, "stars",
            plan["recipient_username"], plan["recipient_user_id"], plan["recipient_name"],
            len(account_ids), accounts_data,
            error_summary, 
