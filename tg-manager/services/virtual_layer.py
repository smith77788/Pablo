...
* bot-to-bot как события Telegram НЕ подтверждён и на нём ничего не строим:
  боты не получают апдейты от других ботов. «Сеть ботов» координируется через
  этот слой на нашей стороне, а не перепиской ботов.
"""
