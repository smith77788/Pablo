...
class GiftTransferService:
    ...
    @staticmethod
    async def add_items_to_plan(pool, plan_id: int, items: list[dict]) -> int:
        ...
        for item in items:
            try:
                ...
                """,
                    ...
                    item["gift_id"],
                    item["gift_type"],
                    item.get("stars_cost", 0),
                )
                count += 1
            except Exception as e:
                log.error(
                    "add_items: failed for inventory_id %d: %s",
    ...
    @staticmethod
    async def _check_payment_source(
        pool, owner_id: int, payment_source: str, method_id: int
    ) -> bool:
        ...
        for acc_id in account_ids:
            ...
            if acc:
                ...
                accounts_data.append(
                    {"account_id": acc_id, "phone": acc["phone"], "gifts_count": count}
                )
