...
async def _keywords_via_ai(description: str, ai_provider: Any) -> list[str]:
    ...
    prompt = (
        ...
        "- Используй ТОЛЬКО слова которые не заблокированы в Telegram поиске\n"
        "- Если ниша чувствительная — используй эвфемизмы и косвенные термины "
        "(например: вместо запрещённых прямых слов используй 'встречи', 'знакомства', "
        "'досуг', 'сопровождение', 'темы', 'общение', 'девушки рядом', "
        "'модели', 'VIP сервис', 'релакс', 'массаж', 'спа', 'развлечения')\n"
    ...
    return _apply_euphemisms(description, keywords[:12])


_EUPHEMISM_MAP: dict[str, list[str]] = {
    # Эскорт / Adult (прямые слова заблокированы в TG поиске → замены)
    "эскорт":       ["знакомства встречи", "сопровождение досуг", "досуг девушки", "VIP сервис", "модели встречи"],
    "индивидуалка": ["знакомства Москва", "встречи досуг", "темы общение", "девушки рядом"],
    "эскортница":   ["модели сопровождение", "досуг встречи", "развлечения VIP"],
    "проститутк":   ["досуг общение", "знакомства темы", "встречи девушки"],
    "шлюх":         ["знакомства быстро", "досуг встречи", "темы развлечения"],
    # Наркотики (заблокированы → эвфемизмы для поиска смежной аудитории)
    "наркотик":     ["химия темы", "клубная жизнь", "вещества форум"],
    "закладк":      ["доставка анонимно", "конфиденциально темы"],
    # Оружие
    "оружи":        ["охота снаряжение", "тактика форум", "стрелковый клуб"],
}


def _apply_euphemisms(description: str, keywords: list[str]) -> list[str]:
    """Заменяет/дополняет ключи эвфемизмами для слов, заблокированных в TG."""
    desc_lower = description.lower()
    extra: list[str] = []
    for trigger, replacements in _EUPHEMISM_MAP.items():
        if trigger in desc_lower:
            extra.extend(replacements)
    if extra:
        return (keywords + extra)[:12]
    return keywords
...
def _keywords_rule_based(description: str) -> list[str]:
    ...
    for w in words:
        if w.lower() not in _STOP and w not in seen:
            seen.append(w)
    keywords = seen[:8]
    return _apply_euphemisms(description, keywords)
