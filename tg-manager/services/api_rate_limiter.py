...
СТАТУС: НЕ ПОДКЛЮЧЁН (проверено 2026-07-27). Лимитер запросов на пользователя
для `mini_app_api`. Подключение осмысленно, но требует решения, что делать при
превышении (429 против тихой очереди) — иначе мини-апп начнёт молча падать.
"""
