        ...
        for b in bots:
            ...
            try:
                rows = await pool.fetch(
                    ...
                    "AND is_blocked=FALSE AND suspect=FALSE", b["bot_id"]
                )
            except Exception:
        ...
        for b in bots:
            ...
            try:
                rows = await pool.fetch(
                    ...
                    "AND is_active=TRUE AND is_blocked=FALSE AND suspect=FALSE",
                    b["bot_id"], lang,
                )
            except Exception:
...
async def _exec_boost_reactions(
    ...
    # зависшей операции сторожем, после перезапуска контейнера. Тогда
    # аккаунты, уже отработавшие цель, отрабатывают её второй раз, а это
    # повторная отправка ТОЙ ЖЕ реакции тем же аккаунтом. Реакция при этом
    # остаётся ровно одна: boost_engine.boost_reaction шлёт SendReactionRequest
    # со списком из одного эмодзи, а он ЗАДАЁТ набор реакций аккаунта, а не
    # переключает его (снятие — это пустой список). Значит накрутка от повтора
    # не растёт, тратятся только вызовы и лимит аккаунта.
    #
    # Единица работы здесь — аккаунт (цель у операции одна), поэтому ключ
    # журнала acc#<id> устойчив к любому порядку и составу отбора.
    _already_done = await completed_targets(pool, op_id)
    ...
    try:
        if isinstance(explicit_ids, list) and explicit_ids:
            ...
            user_ids = [r["user_id"] for r in await pool.fetch(
                "SELECT user_id FROM bot_users WHERE bot_id=$1 AND is_active=TRUE "
                "AND is_blocked=FALSE AND suspect=FALSE" + _seg_sql, int(bot_id)
            )]
    except Exception as exc:
        return {"status": "failed", "summary": f"⚠️ Ошибка получения подписчиков: {exc}"}
