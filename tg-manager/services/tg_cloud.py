...
# Потолок ОДНОЙ загрузки через HTTP. Не то же самое, что квота: квота — сколько
# всего можно хранить, а это — сколько процесс готов принять за раз. store_file
# держит файл в памяти целиком (шифрует и режет там же), и v1-бэкенд кладёт кусок
# в `bytea`, у которого предел 1 ГБ на значение. Без явного потолка первый же
# большой файл валит процесс по памяти, а не отказывает пользователю.
# 50 МБ: на пике store_file держит в памяти сам файл и его шифрованную копию, а
# DB-транспорт пишет каждую реплику отдельной строкой — то есть ~3 объёма файла за
# загрузку. Поднимается INFRAGRAM_CLOUD_MAX_FILE_MB, когда под облаком будет
# флот-транспорт с потоковой заливкой, а не bytea.
MAX_FILE_BYTES = max(1, int(os.getenv("INFRAGRAM_CLOUD_MAX_FILE_MB", "50"))) * 1024 * 1024
...
async def _effective_limit(pool, owner_id: int) -> int:
    """Байтовый потолок хранилища. 0 = безлимит, и он только у владельца платформы.

    Возврат 0 для обычного пользователя означал бы не «нет доступа», а «сколько
    угодно»: `store_file` проверяет квоту под `if limit > 0`, то есть при нуле не
    проверяет её вообще. На это попадал каждый, кто получил доступ по тарифу (pro),
    а не через `set_paid`: строки в tg_cloud_quota у него нет, `paid` False — и он
    хранил неограниченный объём, причём мини-апп писал ему «безлимит». Поэтому у
    всех, кроме админа, потолок положительный: явный, либо дефолтный платный.
    """
    if _is_admin(owner_id):
        return 0
    q = await get_quota(pool, owner_id)
...
async def store_file(pool, owner_id: int, name: str, data: bytes,
    ...
    if size > MAX_FILE_BYTES:
        raise QuotaExceeded(
            f"файл больше предела одной загрузки "
            f"({size // (1024 * 1024)} МБ > {MAX_FILE_BYTES // (1024 * 1024)} МБ)")
