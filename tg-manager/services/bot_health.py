...
def describe(row: dict, now: datetime | None = None) -> dict:
    ...
    if row.get("is_active") is False:
        # Сюда бот попадает двумя путями: владелец убрал его из работы, либо
        # опрос отключил после мёртвого токена. Снаружи это одно и то же
        # состояние, и подпись «выключен ВАМИ» во втором случае была неправдой.
        # Говорим то, что верно в обоих: бот не в работе, данные целы, вернуть
        # можно здесь же.
        return {"state": "off", "kind": "", "label": "⏸ Убран из работы",
                "hint": "Бот не забирает обновления и не виден в списке в боте."
                        " Подписчики, воронки и история сохранены — верните его"
                        " кнопкой «Вернуть в работу» или повторным добавлением"
                        " токена.",
                "attention": False}
