...
СТАТУС: НЕ ПОДКЛЮЧЁН. Модуль (хранилище va_channel_brain + capability) готов и
покрыт тестами, но ещё не вызывается из пути публикации: чтобы гейт срабатывал
автоматически перед постановкой поста, нужен отдельный срез — подключение в
пайплайн публикации (через operation_bus) и экран «Правила» в mini-app. До этого
модуль — строительный блок следующего среза, а не мёртвый код.
