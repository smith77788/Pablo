...
полностью тестируемы. Их вызывает пайплайн публикации ПЕРЕД постановкой в очередь:
repetition_check + brand_check + content-mix = единый quality gate (см. spec §8,
§9, §13/14, §51, §89). Генерация текста и хранение модели проекта — отдельные
срезы; этот слой их проверяет, а не заменяет.
