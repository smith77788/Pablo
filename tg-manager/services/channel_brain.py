...
СТАТУС: НЕ ПОДКЛЮЧЁН. `services/op_worker.py::_exec_mass_publish` — реальный
пайплайн публикации — пока НЕ вызывает repetition_check/brand_check/content-mix
из этого модуля. Причина не в забывчивости: repetition_check нуждается в
recent_texts (истории постов КОНКРЕТНОГО канала), а её негде брать — в схеме нет
таблицы с историей опубликованных постов на канал (content memory), только сам
факт публикации через operation_log. Подключать пустышкой (recent_texts=[]) —
хуже, чем не подключать: quality gate всегда будет отвечать «дубликатов нет»,
создавая ложное чувство защиты. Wiring — отдельная задача: сперва таблица
истории постов на канал + её наполнение в _exec_mass_publish, потом сам gate.
"""
