...
│   └── 📋 Active Tasks       → BmCb(action="active_tasks")   ← r12
├── 📢 Broadcasts
...
- **Импорт существующих каналов** из Telegram в managed_channels
- **Импорт существующих групп** из Telegram в managed_channels
...
- Health Dashboard V2 (health_score тренды, авто-ротация, schema_v45)
- **Topology Map** — граф связей ботов/каналов/аккаунтов (NEW r15)
- **Drift Detection** — мониторинг изменений каналов (NEW r15, schema_v46)
- **Presence Pack System** — воронка бот+каналы+группы (NEW r15, schema_v47)
...
- Background mass_publish с task_registry (не блокирует интерфейс)
...
| Global Presence Factory V1+V2+V3 | ✅ | global_presence.py + geo_data + username_engine |
...
| Account Health Engine V2 | ✅ | services/account_health.py + schema_v45 |
...
1. ✅ **Account Health Dashboard V2** — health_score тренды + улучшенные рекомендации
   - schema_v45: account_health_history для снапшотов health_score
   - health_score в списке аккаунтов + тренд за 7 дней
   - Контекстные рекомендации с health_tips
   - Персистентность health_score в account_health.py

2. ✅ **Авто-ротация аккаунтов** — автоматические кулдауны
   - trust < 0.3 → 72h, trust 0.3–0.6 → 24h
   - Фоновый цикл в trust_engine каждые 6 часов
   - Уведомления владельцам через notify_if_enabled

3. ✅ **Behavioral Engine Enhancement** — anomaly detection v2
   - Velocity anomaly: события/час > 3× от среднего за 7 дней
   - Pattern deviation: attention/ecosystem отклонение > 50% от baseline

4. ✅ **Global Presence Factory V3** — полный пакет (каналы + группы + боты)

### ✅ ЗАКРЫТО (r15)

5. ✅ **Import Center** — CSV import для bulk операций
   - Массовый импорт аккаунтов из CSV (колонки session, cluster)
   - Auto-cluster assignment при импорте
   - Загрузка .txt для Bulk DM и InviteUsers

6. ✅ **Drift Detection** — мониторинг изменений
   - Отслеживание изменений username/title/about в каналах (schema_v46)
   - Алерты при неожиданных изменениях в restriction_events

7. ✅ **Topology Map** — граф связей
   - Визуализация связей ботов/каналов/аккаунтов
   - Статистика перекрёстных ссылок

### ✅ ДОПОЛНИТЕЛЬНО (сверх плана, r15)

- ✅ **Strike Engine V2** — эшелонированная атака (12 векторов, pre-flight, staggered waves)
- ✅ **Presence Pack System** — воронка бот+каналы+группы (schema_v47)
...
- ✅ **Bot Admin Sessions** — /admin TOKEN для владельцев ботов (schema_v47)
- ✅ **UX audit** — Cancel/Back кнопки во всех FSM wizard'ах + валидация ввода

### 🟡 Ожидают определения (r16)

8. Новые приоритеты не назначены — ждём указаний пользователя
...
_Последнее обновление: 2026-05-30 (r15)_
_Следующий build-номер: r16_
