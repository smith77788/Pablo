...
## 16. СЛЕДУЮЩИЕ ПРИОРИТЕТЫ (r12 → r13)
...
2. **Bulk actions — улучшение UX**
   - Настройки задержки (кастомный pacing per операцию)
   - Выбор аккаунтов с preview перед запуском
   - Preview/подтверждение для ВСЕХ массовых операций

3. **UX cleanup — полный аудит ВСЕХ меню**
   - Проверить на button dumps (группы по 3-4 кнопки, не больше)
   - Убедиться что Back/Cancel есть на каждом экране
   - Inline help для сложных полей во всех FSM
...
_Последнее обновление: 2026-05-30 (r12)_
_Следующий build-номер: r13_
