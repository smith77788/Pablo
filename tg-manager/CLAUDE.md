# CLAUDE.md — BotMother OS: Главный ориентир системы (v3.3)
...
## 9. СХЕМА БД (60+ таблиц, v44 schema)
...
## 16. СЛЕДУЮЩИЕ ПРИОРИТЕТЫ (r13 → r14)

### 🔴 Высокий приоритет — ✅ ВСЕ ЗАКРЫТЫ (r13)

1. ✅ **AI Assistant — реальное выполнение команд** (dcb90e6, b1a351b)
   - 9 READ + 8 ACTION инструментов с подтверждением
   - Создание каналов/ботов/групп/массовое через AI
   - Подтверждение перед выполнением + отмена

2. ✅ **Bulk actions — улучшение UX** (b1a351b)
   - 4 пресета темпа: Безопасный/Средний/Быстрый/Турбо
   - Расчётное время выполнения на экране подтверждения
   - Pacing-aware задержки вместо хардкода

3. ✅ **UX cleanup — аудит меню** (01fb200)
   - 5 ключевых меню исправлены (button dumps убраны)
   - Infrastructure, Operations, Bulk Ops, Channel Factory, OpBuilder
   - Группировка по 2 кнопки где возможно

### 🟡 Средний приоритет (следующие задачи)

4. **Global Presence Factory V3** — поддержка ботов + пакеты
   - Создание ботов в гео-распределении
   - Пакетное создание (каналы + группы + боты в каждом городе)
   - CSV import для списка городов

5. **Account Health Dashboard V2**
   - Тренды (графики изменения health_score)
   - Рекомендации для восстановления trust_score
   - Automatic rotation при низком score

6. **Behavioral Engine Enhancement**
   - Fine-tune формулы scoring'а
   - Уникальные паттерны поведения (anomaly detection)

### 🟢 Низкий приоритет

7. Import Center: CSV import для bulk операций
8. Drift Detection: мониторинг изменений в каналах/ботах
9. Topology map: граф связей ботов/каналов/аккаунтов
10. Telegram Mini App: веб-дашборд для таблиц/графиков
11. Approval workflows: подтверждение перед критическими bulk-операциями
...
_Последнее обновление: 2026-05-30 (r13)_
_Следующий build-номер: r14_
