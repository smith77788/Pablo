...
**Изменённые файлы:**
- `bot/states.py` — новый FSM-стейт `Broadcast.waiting_placeholders`
- `bot/handlers/templates.py` — обнаружение `{{KEY}}` + handler для ввода значений + рендеринг

**До:**
- Шаблоны использовались as-is, без подстановки значений
- Плейсхолдеры `{{NAME}}`, `{{DATE}}` и т.д. отправлялись буквально
- Не было возможности персонализировать шаблоны при рассылке

**После:**
- `cb_template_use` обнаруживает `{{KEY}}` в тексте шаблона
- Если плейсхолдеры найдены → запрашивает значения в формате `key=value, key=value`
- Если нет → сразу переход к подтверждению (существующий флоу без изменений)
- `msg_placeholders`: парсит ввод, проверяет что все плейсхолдеры заполнены, рендерит
- Показывает превью с подставленными значениями перед запуском рассылки
- Предупреждение о незаполненных плейсхолдерах после рендеринга

**Проверки:**
- `python3 -c "import ast; ast.parse(open(f).read())"` — все файлы OK
- Функциональный тест: обнаружение → парсинг → рендеринг → проверка missing
- Проверка отсутствия конфликтов с `broadcast.py` handler'ами (разные FSM-стейты)
