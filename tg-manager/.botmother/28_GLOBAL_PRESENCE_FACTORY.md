...
* `🗂 Каталог` — фильтры по тематике, типу актива и статусу с пагинацией.
  Фильтр едет в `callback_data` строкой `role|asset|status`, а не в FSM:
  каталог открывается из отчёта и после перезапуска бота. Значения приходят
  извне → сверяются с белым списком и подставляются параметрами запроса.
  Показываются только те фильтры, которые в проекте есть.
* `📥 Экспорт ссылок` — CSV с ссылками, аккаунтом-создателем и ошибками.
* `gp_bulk_apply` — пакетные операции ПО ОБЪЕКТАМ проекта (значение считается
  для каждого отдельно). Отличается от общего `bulk_edit_channels`, который
  ставит один текст на все каналы аккаунта. Действия:
  `about` / `avatar` / `both` — оформление;
  `post_pin` — пост по шаблону с подстановкой города + закреп;
  `admin` — выдача прав пользователю во всех объектах;
  `username` — доназначение имени тем, у кого не встало.

**Инварианты пакетных операций:**
1. Набор действий у меню (`_BULK_LABELS`) и исполнителя (`GP_BULK_ACTIONS`)
   обязан совпадать — иначе кнопка ставит операцию, которую исполнитель
   отвергает как неизвестную. Стережёт тест.
2. `username` по умолчанию ДОназначает (`only_missing=True`). Массовая смена
   имени у работающих каналов ломает существующие ссылки.
3. Пауза при `username` — 90–180 с: частые `UpdateUsername` Telegram трактует
   как автоматизацию.
4. Отвергнутое Telegram имя резервируется в аллокаторе, иначе оно будет
   предложено следующему объекту с тем же исходом.
5. Неудачный закреп не считается провалом элемента: пост опубликован.
6. Пропуски (карантин, нет сессии) считаются ОТДЕЛЬНО от ошибок.

### Мини-апп

`POST /api/miniapp/global_presence/{plan_id}/bulk_apply` + кнопки оформления
на экране плана; деталь плана отдаёт `dressing` (сколько объектов с аватаром
и username) и фактические username со ссылками.

### Что осталось планом, а не сделано

* Массовая СМЕНА username у уже работающих объектов (доназначение есть;
  смена ломает существующие ссылки, поэтому нужен отдельный режим с явным
  подтверждением и переносом ссылок).
* Пост+закреп и назначение админа из мини-аппа (в боте есть; в мини-аппе
  требуют ввода текста/ID — нужен отдельный экран формы).
* Фильтры каталога в мини-аппе (в боте есть).
