...
TOOL_DEFINITIONS = [
    ...
    {
        "name": "search_memory",
        "description": "Найти сохранённую память Infragram пользователя по ключевым словам. Используй перед ответом, если вопрос связан с прошлым контекстом, настройками, проектами, ошибками или предпочтениями.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "remember",
        "description": "Сохранить важную память Infragram. Используй только когда пользователь явно просит запомнить, сохранить правило, проект, настройку, вывод или устойчивый факт.",
        "input_schema": {
            "type": "object",
            "properties": {
