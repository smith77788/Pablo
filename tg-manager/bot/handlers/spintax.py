...
Как работает:
  ...
  2. Уже подключённые к платформе LLM-провайдеры (OpenRouter/Groq/Gemini/Ollama)
     переписывают его в несколько максимально разных spintax-шаблонов.
...
def _plural_variant(n: int) -> str:
    """Русская форма слова «вариант» для числа n."""
...
_INTRO = (
    ...
    f"<b>{_SPIN_COUNT}</b> максимально разных spintax-{_plural_variant(_SPIN_COUNT)} "
