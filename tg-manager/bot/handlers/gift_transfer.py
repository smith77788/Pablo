...
def make_gt_kb(*rows) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [make_gt_button(**btn) if isinstance(btn, dict) else btn for btn in row]
            for row in rows
        ]
    )
...
@router.callback_query(
    F.data.startswith("gt:toggle_acc:"), GiftTransferFSM.selecting_accounts
)
...
async def cb_start_scan(callback: CallbackQuery, state: FSMContext, pool):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⏳ Обработка...", callback_data="gt:ignore"
                    )
                ]
            ]
        ),
    ...
    async def _scan_bg() -> None:
        try:
            gifts = await GiftInventoryService.scan_multiple_accounts(
                pool, user_id, account_ids
            )
            synced = await GiftInventoryService.sync_inventory_to_db(
                pool, user_id, gifts
            )
...
async def cb_start_transfer(callback: CallbackQuery, state: FSMContext, pool):
    ...
    if not accounts_with_gifts:
        await callback.answer(
            "Подарки не найдены. Сначала выполните сканирование.", show_alert=True
        )
        return
...
@router.callback_query(
    F.data.startswith("gt:transfer_toggle:"), GiftTransferFSM.selecting_accounts
)
...
@router.callback_query(
    F.data == "gt:enter_username", GiftTransferFSM.selecting_recipient
)
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="◀️ Отмена", callback_data="gt:select_recipient"
                    )
                ]
            ]
        ),
...
@router.callback_query(
    F.data.startswith("gt:use_recipient:"), GiftTransferFSM.selecting_recipient
)
...
@router.callback_query(
    F.data.startswith("gt:payment_"), GiftTransferFSM.selecting_payment
)
...
async def cb_confirm_transfer(callback: CallbackQuery, state: FSMContext, pool):
    ...
    if not validation["valid"]:
        await callback.answer(
            f"Ошибка проверки: {validation['errors']}", show_alert=True
        )
        return
    ...
    op_id = await _op_bus.submit(
        pool, user_id, "gift_transfer", {"plan_id": plan_id}, total_items=len(items)
    )
...
async def _show_transfer_preview(message, state, user_id, pool):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(
        text="✅ Подтвердить передачу подарков", callback_data="gt:preview_confirm"
    )
    kb.row()
...
async def cb_check_progress(callback: CallbackQuery, state: FSMContext, pool):
    ...
    if stats["failed"] > 0:
        ...
        if retryable:
            kb.button(
                text=f"🔁 Повторить ({len(retryable)} неудачных)",
                callback_data="gt:retry_failed",
            )
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🔄 Проверить прогресс", callback_data="gt:check_progress"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="◀️ В главное меню", callback_data="gt:main"
                    )
                ],
            ]
        ),
...
async def cb_view_report(callback: CallbackQuery, state: FSMContext, pool):
    ...
    if report.get("retryable_failures"):
        report_text += (
            f"⚠️ <b>Повторяемые ошибки:</b> {len(report['retryable_failures'])}\n"
        )
...
async def cb_manage_recipients(callback: CallbackQuery, state: FSMContext, pool):
    ...
    recipients_text = (
        "\n".join(f"• {r['name']} — {r['username'] or 'нет @'}" for r in recipients)
        if recipients
        else "Нет сохранённых получателей."
    )

    await callback.message.edit_text(
        f"👥 <b>Сохранённые получатели</b>\n\n{recipients_text}",
        reply_markup=kb.as_markup(),
    )
