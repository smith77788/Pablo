    ...
    for acc in accounts:
        ...
        for i in range(bot_count):
            ...
            result = await account_manager.create_bot_via_botfather(
                acc["session_str"], data["bot_name"], username, _acc=acc
            )
            if "error" in result:
...
async def fsm_bulk_channel_id(message: Message, state: FSMContext, pool: asyncpg.Pool) -> None:
    ...
    if op == "leave":
        ...
        for idx, acc in enumerate(accounts):
            label = html.escape(acc["first_name"] or acc["phone"])
            try:
                ok = await account_manager.leave_channel(acc["session_str"], channel_ref, _acc=acc)
...
async def fsm_bulk_post_text(message: Message, state: FSMContext, pool: asyncpg.Pool) -> None:
    ...
    if op == "post" and selected_ids:
        ...
        for idx, acc in enumerate(accounts):
            label = html.escape(acc["first_name"] or acc["phone"])
            result = await account_manager.post_to_channel(acc["session_str"], channel_ref, text_to_post, access_hash=bulk_access_hash, _acc=acc)
            if "msg_id" in result:
