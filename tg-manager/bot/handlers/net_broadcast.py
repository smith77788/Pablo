...
async def cb_net_bc_segment(callback: CallbackQuery, callback_data: NetBcCb,
...
async def cb_net_bc_type_msg(callback: CallbackQuery, callback_data: NetBcCb,
                              pool: asyncpg.Pool, state: FSMContext) -> None:
    await callback.answer()
    segment = callback_data.segment
...
async def msg_net_bc_text(message: Message, state: FSMContext, pool: asyncpg.Pool) -> None:
...
async def cb_net_bc_confirm(callback: CallbackQuery, callback_data: NetBcCb,
    ...
    if segment == "all_each":
        for bot in bots:
            ...
            if ids:
                bc_id = await db.create_broadcast(pool, bot["bot_id"], text, len(ids),
                                                   callback.from_user.id)
                broadcaster.start(pool, http, bc_id, bot["token"], bot["bot_id"],
                                   text, None, ids, None)
                total_started += 1
                total_users += len(ids)
        ...
        for bot_id, ids in by_bot.items():
            bc_id = await db.create_broadcast(pool, bot_id, text, len(ids),
                                               callback.from_user.id)
            broadcaster.start(pool, http, bc_id, token_map[bot_id], bot_id,
                               text, None, ids, None)
            total_started += 1
            total_users += len(ids)
        ...
        for bot in bots:
            ids = await db.get_inactive_user_ids(pool, bot["bot_id"], days_from, days_to)
            if ids:
                bc_id = await db.create_broadcast(pool, bot["bot_id"], text, len(ids),
                                                   callback.from_user.id)
                broadcaster.start(pool, http, bc_id, bot["token"], bot["bot_id"],
