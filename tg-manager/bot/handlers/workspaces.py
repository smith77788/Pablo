...
def _ws_view_kb(ws_id: int, is_owner: bool) -> ...:
    ...
    if is_owner:
        kb.button(
            text="👥 Участники",
            callback_data=WorkspaceCb(action="members", ws_id=ws_id),
        )
        kb.button(
            text="🔗 Пригласить",
            callback_data=WorkspaceCb(action="invite", ws_id=ws_id),
        )
        kb.button(
            text="🗑 Удалить", callback_data=WorkspaceCb(action="leave", ws_id=ws_id)
        )
    else:
        kb.button(
            text="👥 Участники",
        ...
        kb.button(
            text="🚪 Покинуть", callback_data=WorkspaceCb(action="leave", ws_id=ws_id)
        )
...
async def cb_ws_members(
    ...
    for m in members:
        ...
        role_icon = {"owner": "👑", "admin": "⚙️", "member": "👤", "viewer": "👁"}.get(
            m["role"], "❓"
        )
...
async def msg_ws_invite_code(
    ...
    if not code or len(code) > 100:
        ...
        kb.button(text="❌ Отмена", callback_data=WorkspaceCb(action="menu"))
        await message.answer(
            "⚠️ Введите корректный код приглашения (до 100 символов).",
            reply_markup=kb.as_markup(),
        )
        return
    await state.clear()
