...
Исправлено: да (в работе, поэтапно). Введён единый источник `bot/utils/tariffs.py` (env-оверрайд на каждый лимит/квоту/фичу/цену, именованный `UNLIMITED` вместо `9999`); `subscription.py` теперь проецирует его без смены дефолтов (free 5/5, paid ∞, $29 из env). Регресс: `tests/test_tariffs.py`. Дальше: единая функция активации подписки, аудит принудительности фича-гейтов.
