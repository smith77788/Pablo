...
ОТКРЫТО (продуктовое решение, не баг): `check_operation_limit`/месячные квоты операций всё ещё не вызываются — сейчас операции строго платные (min_plan), поэтому free-квоты в `_OPERATION_LIMITS` инертны и не противоречат поведению, но и не активны. Нужно решение владельца: (A) операции строго платные (тогда free-квоты убрать/обнулить) или (B) free получает малую квоту некоторых операций (тогда снизить их min_plan до free и включить `check_operation_limit` в `submit()`). Пока не трогаю — это меняет выручку/доступ.
