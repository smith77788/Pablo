...
ОТКРЫТО (решение за владельцем, деплой-риск): `requirements.txt` жёстко пинит
aiogram==3.13.1 (сент. 2024) при актуальной 3.30.0 / Bot API 9.4 — новые
возможности Bot API недоступны. Код новых фич НЕ использует (проверено: из свежего
API ничего не импортируется; Gift в коде — Telethon-домен, не Bot API), поэтому
прод не сломан. Но песочница гоняет 3.29.1, а прод поставит 3.13.1 — сам по себе
дрейф «тестируем не то, что запускаем». Бамп фреймворка через 17 минорных версий
без интеграционного прогона живого бота — не рутинная правка, не делал молча.
