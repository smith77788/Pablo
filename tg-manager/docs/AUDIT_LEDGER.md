...
ОТКРЫТО (нужно решение по продукту): (a) `check_operation_limit` не вызывается нигде — месячные квоты операций мертвы; (b) `operation_bus.submit()` не enforced `min_plan` (гейт только в хендлерах); (c) `OP_REGISTRY.min_plan` (операции = платные) противоречит ненулевым free-квотам в `_OPERATION_LIMITS`. Требуется решение: операции строго платные или free получает малую квоту — затем централизовать enforcement в `submit()`.
