...
15. **Строка из JSON прямо в TIMESTAMPTZ = фича мертва.** asyncpg биндит параметр
    по типу колонки и на `str` для timestamptz кидает DataError → хендлер падает 500
    ВСЕГДА (фича есть в UI, но не работала никогда). Спасает только явный
    `$N::timestamptz` (тогда парсит Postgres) ИЛИ разбор в `datetime` на границе.
    Так умерли CRM-напоминание и schedule_post. Свип: найти timestamptz-колонки в
    схемах → проверить каждое присваивание `col = $N` без `::` → что туда биндится,
    `datetime` или строка из `body.get()`. Прямой INSERT в operation_queue — двойной
    запах: и обход шины, и обычно без каста (operation_bus.submit кастует сам).
