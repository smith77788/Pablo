...
## МОДУЛЬ Каналы — сигнал активности last_post_at + разблокировка auto_remove (4B) — 2026-07-12
Проверено: почему auto_remove_dead_channels был неисполним. Ранее опирался на `tg_channels.last_post_at` — колонки нет НИГДЕ в схеме (grep по всем schema*.sql пуст) → сигнала «мёртвого канала» не существовало.
Найдено/сделано: заведён реальный сигнал активности. `managed_channels.last_post_at TIMESTAMPTZ` — self-heal при старте (mini_app_api ADD COLUMN IF NOT EXISTS) + проставляется при РЕАЛЬНОЙ публикации в `op_worker._exec_mass_publish` (best-effort UPDATE по owner_id+channel_id на успешном посте). `auto_remove_dead_channels` переписан: членство из `ecosystem_members` (object_type='channel'), активность из `managed_channels.last_post_at`, DELETE из ecosystem_members. Консервативная семантика: `last_post_at IS NULL` (ни одной публикации) → НЕ удаляем («неизвестно» ≠ «мёртвый», свежий канал защищён). Подключён в opt-in цикл `run_auto_management` рядом с auto_add. Регресс `tests/test_channel_activity_signal.py` (4). Осталось (бэклог): проставлять last_post_at и из других путей публикации (quick_post/broadcast/autopost), сейчас только mass_publish — самый частый.

## МОДУЛЬ Ecosystem — полный ретайр мёртвой таблицы ecosystem_channels — 2026-07-12 (ЗАКРЫТО)
Файлы: `mini_app_api.py` (channels-list ~1242/1259), `ecosystem_brain.py` (auto_post_scheduling, auto_remove_dead_channels). Завершение перевода всех читателей `ecosystem_channels` → канонической `ecosystem_members`.
Найдено/исправлено: (1) **channels-list** (ранее «найдено, не исправлено») — ветка «каналы из экосистем» читала мёртвую `ecosystem_channels` И сравнивала `managed_channels.id` (PK) с Telegram `channel_id`. Исправлено в обеих копиях (SELECT+COUNT): `channel_id IN (SELECT em2.object_id FROM ecosystem_members em2 JOIN ecosystems e ... WHERE em2.object_type='channel' AND (e.owner_id=$1 OR e.id IN (...)))`. (2) **auto_post_scheduling** — переведён с `tg_channels JOIN ecosystem_channels` на канонический `managed_channels JOIN ecosystem_members (object_type='channel', object_id=channel_id)`; заодно `tg_channels` НЕ имеет нужных полей — managed_channels правильнее. (3) **auto_remove_dead_channels** — исправлена опечатка в логе (`capture_dna_from_ecosystem` → своё имя). Регресс `tests/test_ecosystem_dead_table.py` (3).
НЕ исправлено сознательно: `auto_remove_dead_channels` (мёртвый, не wired) остался на `ecosystem_channels` — потому что его SELECT опирается на `tg_channels.last_post_at`, а такой колонки НЕТ НИГДЕ в схеме (проверено grep по всем schema*.sql). Функция сломана вдвойне и заблокирована отсутствием сигнала активности канала — корректно реализовать «мёртвый канал» сейчас не на чем. Нужен либо новый столбец активности (last_post_at на managed_channels + его заполнение при постинге), либо запрос к Telegram. Отдельный заход по объёму.
