...
async def main() -> None:
    ...
    for _ddl in (
        ...
        # Контакты: пометка «личный/исключён из работы» (schema_v211). Читается
        # _segment_where на рабочем пути — при лаге миграции путь падал бы.
    ...
    try:
        ...
        # Сначала — корректно свернуть воркер операций, и только потом закрывать
        # пул. Обратный порядок означал бы, что уборка (возврат операций в
        # очередь и снятие аренды аккаунтов) идёт по уже закрытому пулу, то есть
        # не идёт вовсе: после каждого деплоя флот оставался занятым до
        # истечения TTL аренды, а живые операции теряли бюджет живучести.
        try:
            await op_worker.shutdown(pool)
        except Exception:
            log.warning("Корректная остановка воркера операций не удалась", exc_info=True)
        await pool.close()
        await http.close()
